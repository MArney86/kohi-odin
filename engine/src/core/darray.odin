...
_darray_make :: proc($T: typeid) -> [dynamic]T {
    ...
    stats.total_allocated += (DEFAULT_DARRAY_CAPACITY * size_of(T)) + size_of([dynamic]T)
    stats.tagged_allocations[cast(int)types.memory_tag.MEMORY_TAG_DARRAY] += (DEFAULT_DARRAY_CAPACITY * size_of(T)) + size_of([dynamic]T)
    return array
}
...
_darray_reserve :: proc(array: ^[dynamic]$T) -> (ok: bool) {
    ...
    stats.total_allocated += DARRAY_RESIZE_FACTOR * size_of(T)
    stats.tagged_allocations[cast(int)types.memory_tag.MEMORY_TAG_DARRAY] += DARRAY_RESIZE_FACTOR * size_of(T)
    return TRUE
}
...
_darray_push :: proc(array: ^[dynamic]$T, value: T) ->  bool {
    ...
    if cap(array) > start_cap {
        ...
        stats.total_allocated += cast(u64)((cap(array) - start_cap) * size_of(T))
        stats.tagged_allocations[cast(int)types.memory_tag.MEMORY_TAG_DARRAY] += cast(u64)((cap(array) - start_cap) * size_of(T))
        stats.tagged_allocations[cast(int)types.memory_tag.MEMORY_TAG_DARRAY_USED] += cast(u64)(size_of(T))
    } else {
        stats.tagged_allocations[cast(int)types.memory_tag.MEMORY_TAG_DARRAY_USED] += cast(u64)(size_of(T))
    }
...
_darray_pop_at :: proc(array: ^[dynamic]$T, index: u64) -> T {
    ...
    stats.tagged_allocations[cast(int)types.memory_tag.MEMORY_TAG_DARRAY_USED] -= size_of(T)
    return temp
}
...
_darray_insert_at :: proc(array: ^[dynamic]$T, index: u64, value: T) -> bool {
    ...
    if cap(array) > start_cap {
        // Capacity increased, update stats
        stats.total_allocated += (cap(array) - start_cap) * size_of(T)
        stats.tagged_allocations[cast(int)types.memory_tag.MEMORY_TAG_DARRAY] += (cap(array) - start_cap) * size_of(T)
        stats.tagged_allocations[cast(int)types.memory_tag.MEMORY_TAG_DARRAY_USED] += size_of(T)
    } else {
        stats.tagged_allocations[cast(int)types.memory_tag.MEMORY_TAG_DARRAY_USED] += size_of(T)
    }
...
Kdarray_clear :: proc(array: ^[dynamic]$T) {
    ...
    stats.tagged_allocations[cast(int)types.memory_tag.MEMORY_TAG_DARRAY_USED] -= (u64(orig_len) * size_of(T))
}