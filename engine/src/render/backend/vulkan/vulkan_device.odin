...
select_physical_device :: proc(vk_context : ^types.vulkan_context) -> bool {
    ...
    for i:u32=0; i<physical_device_count; i+=1 {
        ...
        darray.push(extensions_temp, typeid_of(cstring), &ext_name)
        ...
        darray.delete(cast(rawptr)requirements.device_extension_names, typeid_of(cstring))
    }
...
physical_device_meets_requirements :: proc(
    ...
    queue_families := darray.make(typeid_of(vk.QueueFamilyProperties))
    darray.reserve(cast(rawptr)queue_families, typeid_of(vk.QueueFamilyProperties), cast(u64)queue_family_count)
    defer darray.delete(cast(rawptr)queue_families, typeid_of(vk.QueueFamilyProperties))
    raw := cast(^runtime.Raw_Dynamic_Array)queue_families
    darray.set_len(queue_families, typeid_of(vk.QueueFamilyProperties), cast(u64)queue_family_count)
    vk.GetPhysicalDeviceQueueFamilyProperties(device, &queue_family_count, cast([^]vk.QueueFamilyProperties)raw.data)

    // look at each queue and see what queues it supports
    logger.INFO("Graphics   | Present   | Compute   | Transfer  | Name")
    min_transfer_score: u8 = 255
    queue_families_slice := cast([^]vk.QueueFamilyProperties)raw.data
    for i:u32=0; i<queue_family_count; i+=1 {
        current_transfer_score: u8 = 0
        //Graphics?
        if queue_families_slice[i].queueFlags & {.GRAPHICS} != {} {
            out_queue_info.graphics_family_index = cast(i32)i
            current_transfer_score += 1
        }

        //Compute?
        if queue_families_slice[i].queueFlags & {.COMPUTE} != {} {
            out_queue_info.compute_family_index = cast(i32)i
            current_transfer_score += 1
        }

        //Transfer?
        if queue_families_slice[i].queueFlags & {.TRANSFER} != {} {
    ...
    if (!requirements.graphics || (requirements.graphics && out_queue_info.graphics_family_index != -1)) &&
       ...
       (!requirements.transfer || (requirements.transfer && out_queue_info.transfer_family_index != -1)) {
        ...
        if requirements.device_extension_names != nil {
            ...
            available_extensions := darray.make(typeid_of(vk.ExtensionProperties))
            vk.CHECK(vk.EnumerateDeviceExtensionProperties(device, nil, &available_extension_count, nil))
            if available_extension_count != 0 {
                expanded := darray.reserve(available_extensions, typeid_of(vk.ExtensionProperties), cast(u64)available_extension_count)
                if !expanded {
                    logger.FATAL("Failed to allocate array for device extensions.")
                    return false
                }
                darray.set_len(available_extensions, typeid_of(vk.ExtensionProperties), cast(u64)available_extension_count)
                ...
                for i:u32=0; i<required_extension_count; i+=1 {
                    ...
                    for j:u32=0; j<available_extension_count; j+=1 {
                        ext_props := cast(^vk.ExtensionProperties)(uintptr(raw.data) + uintptr(j * size_of(vk.ExtensionProperties)))
                        if str.equals(requirements.device_extension_names[i], str.bytes_to_cstr(ext_props.extensionName[:])) {
                            found = true
                            break
                        }
                    }
...
device_create :: proc(vk_context : ^types.vulkan_context) -> bool {
    ...
    pool_create_info.sType = vk.StructureType.COMMAND_POOL_CREATE_INFO
    pool_create_info.queueFamilyIndex = cast(u32)vk_context.device.graphics_queue_index
    pool_create_info.flags = vk.CommandPoolCreateFlags{.RESET_COMMAND_BUFFER}

    vk.CHECK(vk.CreateCommandPool(vk_context.device.logical_device, &pool_create_info, vk_context.allocator, &vk_context.device.graphics_command_pool))
    logger.INFO("Created graphics command pool successfully.")
...
device_destroy :: proc(vk_context : ^types.vulkan_context) {
    ...
    vk_context.device.transfer_queue = {}

    // Destroy command pool
    logger.INFO("Destroying command pools...")
    vk.DestroyCommandPool(vk_context.device.logical_device, vk_context.device.graphics_command_pool, vk_context.allocator)
    ...
    if vk_context.device.swapchain_support.formats != nil {
        darray.delete(cast(rawptr)vk_context.device.swapchain_support.formats, typeid_of(vk.SurfaceFormatKHR))
    ...
    if vk_context.device.swapchain_support.present_modes != nil {
        darray.delete(cast(rawptr)vk_context.device.swapchain_support.present_modes, typeid_of(vk.PresentModeKHR))
...
device_query_swapchain_support :: proc(
    ...
    if out_support_info.format_count != 0 {
        if out_support_info.formats == nil {
            formats := darray.make(typeid_of(vk.SurfaceFormatKHR))
            out_support_info.formats = cast(^[dynamic]vk.SurfaceFormatKHR)formats
            out_test := darray.reserve(formats, typeid_of(vk.SurfaceFormatKHR), cast(u64)out_support_info.format_count)
            if !out_test{
                logger.FATAL("Failed to allocate properly sized array for swapchain formats.")
                return
            }
            darray.set_len(formats, typeid_of(vk.SurfaceFormatKHR), cast(u64)out_support_info.format_count)
        }
        raw := cast(^runtime.Raw_Dynamic_Array)out_support_info.formats
        vk.CHECK(vk.GetPhysicalDeviceSurfaceFormatsKHR(physical_device, surface, &out_support_info.format_count, cast([^]vk.SurfaceFormatKHR)raw.data))
    ...
    if out_support_info.present_mode_count != 0 {
        if out_support_info.present_modes == nil {
            present_modes := darray.make(typeid_of(vk.PresentModeKHR))
            out_support_info.present_modes = cast(^[dynamic]vk.PresentModeKHR)present_modes
            if !darray.reserve(present_modes, typeid_of(vk.PresentModeKHR), cast(u64)out_support_info.present_mode_count) {
                logger.FATAL("Failed to reserve capacity for swapchain present modes.")
                return
            }
            darray.set_len(present_modes, typeid_of(vk.PresentModeKHR), cast(u64)out_support_info.present_mode_count)
        }
        ...
        vk.CHECK(vk.GetPhysicalDeviceSurfacePresentModesKHR(physical_device, surface, &out_support_info.present_mode_count, 
                                                            cast([^]vk.PresentModeKHR)raw.data))
    }
}
