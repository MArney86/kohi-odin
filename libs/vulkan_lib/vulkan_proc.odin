...
// Types copied from vendor:vulkan functions to be linked from installed system Vulkan SDK
...
load :: proc () -> bool {
    ...
    when os.OS == .Windows {
        ...
        library_name = "libvulkan.so.1"
    }/* else if os.OS == .macos {
        library_name = "libvulkan.dylib"
    }*/ else {
        fmt.eprintf("Unsupported OS for Vulkan library loading\n")
        return false
    }

    vulkan_library, ok := dynlib.load_library(library_name)
    if !ok {
        return false
    }

    vk_get_instance_proc_addr_ptr, found := dynlib.symbol_address(vulkan_library, "vkGetInstanceProcAddr")
    if !found {
        return false
    }

    load_proc_addresses_global(vk_get_instance_proc_addr_ptr)
